...
async function getBet() {
  ...
  if(indice!=""){
    ...
    const persona = await contractR.methods.getBet(indice).call();
    ...
    console.log('Nombre:', persona[2]);
    console.log('Correo:', persona[3])
    console.log('Date:', persona[4])
    console.log('Dirección:', persona[5]);
  } catch (error) {
...
const setLog = async () => {
  ...
  var content = document.getElementById('bet').value
  var optionA = document.getElementById('resultA').value
  var optionB = document.getElementById('resultB').value
  var date = document.getElementById('date').value

  if(nombre!=""&&content!=""){

     const result = await contractR.methods.setBet("Bet",nombre,content,optionA,optionB,date)
